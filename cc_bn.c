...
{
    int i;
    cc_bn_word_t carry = A[bn_word_len - 1] >> (CC_BN_WORD_BITS - 1);
    for (i = bn_word_len - 1; i > 0; i -= 1)
    {
...
{
    size_t i;
    cc_bn_word_t borrow = 0;
    for (i = 0; i < bn_word_len; i++)
    {
        cc_bn_word_t z = (A[i] < borrow);
        cc_bn_word_t t = A[i] - borrow;
        borrow = (t < B[i]) + z;
        R[i] = t - B[i];
    }

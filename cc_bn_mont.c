...
// R canot alias N
void cc_bn_mont_RR(cc_bn_word_t *RR, const cc_bn_word_t *N, size_t N_word_len)
{
    size_t i;
...
// return carry
// R can alias A
cc_bn_word_t cc_bn_mul_word_add_ret(cc_bn_word_t *R, const cc_bn_word_t *A, size_t bn_word_len, cc_bn_word_t d)
{
    size_t i;
...
// A, b < N, N must be odd
// D cannot alias A
void cc_bn_core_mont_mul_word(cc_bn_word_t *D, const cc_bn_word_t *A, const cc_bn_word_t b, const cc_bn_word_t *N, size_t bn_word_len, cc_bn_word_t Ni)
{
    cc_bn_word_t a0 = A[0];
    ...
    // q = (d0 + a0 *bi) * Ni
    cc_bn_word_t q = (D[0] + a0 * b) * Ni;

    // d = d + A * B[i]
    cc_bn_word_t carry1 = cc_bn_mul_word_add_ret(D, A, bn_word_len, b);
    // d = d + q * N
    cc_bn_word_t carry2 = cc_bn_mul_word_add_ret(D, N, bn_word_len, q);

    cc_bn_word_t Rn = carry1 + carry2;
    cc_bn_word_t carry = Rn < carry1;
...
void cc_bn_core_mont_mul(cc_bn_word_t *D, const cc_bn_word_t *A, const cc_bn_word_t *B, const cc_bn_word_t *N, size_t bn_word_len, cc_bn_word_t Ni)
{
    cc_bn_word_t a0 = A[0];
    ...
    {
        // q = (d0 + a0 *bi) * Ni mod k
        cc_bn_word_t q = (D[0] + a0 * B[i]) * Ni;

        // d = d + A * B[i]
        cc_bn_word_t carry1 = cc_bn_mul_word_add_ret(D, A, bn_word_len, B[i]);
        // d = d + q * N
        cc_bn_word_t carry2 = cc_bn_mul_word_add_ret(D, N, bn_word_len, q);

        cc_bn_word_t Rn = carry1 + carry2;
        cc_bn_word_t c1 = Rn < carry1;
        Rn = Rn + carry;
        cc_bn_word_t c2 = Rn < carry;
        carry = c1 | c2;
...
cc_status_t cc_bn_mont_sqrt_p3(cc_bn_word_t *R, const cc_bn_word_t *A, const cc_bn_word_t *P, size_t bn_word_len, cc_bn_word_t Ni)
{
    ...
    cc_bn_word_t T[CC_BN_MAX_WORDS];
    cc_bn_word_t R1[CC_BN_MAX_WORDS];

...
// 辅助函数：比较两个大数数组是否相等
int cc_bn_compare(const cc_bn_word_t *a, const cc_bn_word_t *b, size_t len)
{
    return memcmp(a, b, len * sizeof(cc_bn_word_t));
}
...
void test_cc_bn_rshift()
{
    ...
    {
        printf("测试用例2: 移位0位\n");
        cc_bn_word_t input[] = {0x12345678, 0x9ABCDEF0, 0xFEDCBA98};
    ...
    {
        printf("测试用例3: 按字边界移位\n");
        cc_bn_word_t input[] = {0x12345678, 0x9ABCDEF0, 0xFEDCBA98, 0x11223344};
        cc_bn_word_t output[4];
        cc_bn_word_t expected[] = {0x9ABCDEF0, 0xFEDCBA98, 0x11223344, 0};

        cc_bn_rshift(output, input, 4, 32); // 右移32位（1个字）
        assert(cc_bn_compare(output, expected, 4) == 0);
        printf("通过: 右移32位\n");

        cc_bn_word_t expected2[] = {0xFEDCBA98, 0x11223344, 0, 0};
    ...
    { // 9ABCDEF012345678
        ...
        cc_bn_word_t expected1[] = {0x01234567, 0x09ABCDEF};
        assert(cc_bn_compare(output, expected1, 2) == 0);
        printf("通过: 右移4位\n");

        // 右移1位
        cc_bn_rshift(output, input, 2, 1);
        cc_bn_word_t expected2[] = {0x091A2B3C, 0x4D5E6F78};
    ...
    {
        printf("测试用例6: 移位31位\n");
        cc_bn_word_t input[] = {0xFFFFFFFF, 0x80000000};
        cc_bn_word_t output[2];

        cc_bn_rshift(output, input, 2, 31);
        // 0xFFFFFFFF >> 31 = 0x1，加上0x80000000的最低位0 << 1 = 0x1
        // 0x80000000 >> 31 = 0x1
        cc_bn_word_t expected[] = {0x00000001, 0x00000001};
        assert(cc_bn_compare(output, expected, 2) == 0);
        printf("通过: 右移31位\n\n");
    }
    ...
    // 测试用例8: 全1输入
    {
        ...
        // 每个字右移8位，需要从高位借8位
        cc_bn_word_t expected[] = {0xFFFFFFFF, 0x00FFFFFF};
        assert(cc_bn_compare(output, expected, 2) == 0);
        printf("通过: 全1输入右移8位\n\n");
    }
    ...
    { // FEDCBA98 9ABCDEF0 12345678
        ...
        // 相当于先右移32位，再右移4位
        cc_bn_word_t expected[] = {0x89ABCDEF, 0x0FEDCBA9, 0};
        assert(cc_bn_compare(output, expected, 3) == 0);
        printf("通过: 右移36位\n\n");
    }
